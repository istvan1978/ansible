...
CATALINA_OPTS="${START_ATL_confluence_java_opts} ${CATALINA_OPTS}"
CATALINA_OPTS="-Dconfluence.context.path=${CONFLUENCE_CONTEXT_PATH} ${CATALINA_OPTS}"

#{{ ansible_managed }} START
CATALINA_OPTS="-Xms{{ATL_confluence_java_opts_Xms}}m -Xmx{{ATL_confluence_java_opts_Xmx}}m -XX:+UseG1GC ${CATALINA_OPTS}"
CATALINA_OPTS="-Duser.timezone=Europe/Berlin ${CATALINA_OPTS}"
CATALINA_OPTS="-Dhttps.proxyHost={{ ATL_out_proxy_ip }} -Dhttps.proxyPort={{ ATL_out_proxy_port }} -Dhttp.nonProxyHosts=*.t-systems.com\|*.telekom.de\|*.t-systems-service.com\|localhost\|127.0.0.1\|{{ ATL_proxy_ip }} ${CATALINA_OPTS}"
CATALINA_OPTS="-Dhttps.protocols=TLSv1.2 ${CATALINA_OPTS}"
#{{ ansible_managed }} END
