...
#{{ ansible_managed }} START
JIRA_HOME="{{ATL_jira_base_dir}}/data"
export JAVA_HOME="{{ATL_java_base_dir}}/jdk"
export JDK_HOME="{{ATL_java_base_dir}}/jdk"
#{{ ansible_managed }} END
...
#
#{{ ansible_managed }} START
JVM_MINIMUM_MEMORY="{{ATL_jira_java_opts_Xms}}m"
JVM_MAXIMUM_MEMORY="{{ATL_jira_java_opts_Xmx}}m"
#{{ ansible_managed }} END
...
#{{ ansible_managed }} START
export JAVA_OPTS="-Duser.timezone=Europe/Berlin -Duser.language=de -Duser.country=DE"
export JAVA_OPTS="${JAVA_OPTS} -Dhttps.proxyHost={{ ATL_out_proxy_ip }} -Dhttps.proxyPort={{ ATL_out_proxy_port }} -Dhttp.nonProxyHosts=*.t-systems.com\|*.t-systems.ru\|*.telekom.de\|*.t-systems-service.com\|localhost\|127.0.0.1\|{{ ATL_proxy_ip }}"
...
JAVA_OPTS="-Xms${JVM_MINIMUM_MEMORY} -Xmx${JVM_MAXIMUM_MEMORY} ${JAVA_OPTS} ${JVM_REQUIRED_ARGS} ${DISABLE_NOTIFICATIONS} ${JVM_SUPPORT_RECOMMENDED_ARGS} ${JVM_EXTRA_ARGS} ${JIRA_HOME_MINUSD} ${START_ATL_jira_java_opts}"
